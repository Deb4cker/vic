...
import com.github.deb4cker.vic.uxfreader.parser.uxfparser.JavaSyntaxHelper;
...
@DisplayName("JavaSyntaxHelper")
class UXFParserStaticHelpersTest {
    ...
    class SwitchVisibility {
        ...
        void plusAddsPublic() {
            ...
            JavaSyntaxHelper.switchVisibility(m, "+");
        ...
        void invalidSymbolThrows() {
            MethodDeclaration m = newMethod();
            assertThrows(IllegalArgumentException.class, () -> JavaSyntaxHelper.switchVisibility(m, "?"));
        }
    }
    ...
    class SwitchReturnStatement {
        ...
        void intReturnsZero() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("int");
            assertNotNull(stmt);
            assertTrue(stmt.toString().contains("0"), "Esperado 'return 0;', mas foi: " + stmt);
        }

        @Test
        @DisplayName("'double' retorna ReturnStmt com DoubleLiteralExpr(0.0)")
        void doubleReturnsZeroPoint0() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("double");
            assertTrue(stmt.toString().contains("0"), "Esperado 'return 0.0;', mas foi: " + stmt);
        }

        @Test
        @DisplayName("'boolean' retorna ReturnStmt com false")
        void booleanReturnsFalse() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("boolean");
            assertTrue(stmt.toString().contains("false"), "Esperado 'return false;', mas foi: " + stmt);
        }

        @Test
        @DisplayName("'char' retorna ReturnStmt com literal char nulo")
        void charReturnsNullChar() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("char");
            assertNotNull(stmt);
        }
        ...
        void objectTypeReturnsNull() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("String");
            assertTrue(stmt.toString().contains("null"), "Esperado 'return null;', mas foi: " + stmt);
        }

        @Test
        @DisplayName("'long' retorna ReturnStmt com 0")
        void longReturnsZero() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("long");
            assertTrue(stmt.toString().contains("0"), "Esperado 'return 0;', mas foi: " + stmt);
        }
        ...
        void shortReturnsZero() {
            ReturnStmt stmt = JavaSyntaxHelper.switchReturnStatement("short");
